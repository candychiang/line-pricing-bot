...
def clean_reply(text: str) -> str:
    """
    移除 AI 回覆中的計算過程區塊。
    策略1：找到「您好！以下是您的詢價結果」或「📦 貨物資訊」，從那裡開始輸出。
    策略2：若找不到，逐行刪除計算過程的區塊標題與其內容。
    """
    # 策略1：從標準開頭截斷前面的雜訊
    ...
    for line in lines:
        is_bad_title = any(re.search(p, line) for p in block_title_patterns)
        if is_bad_title:
            skip = True
            continue
        # 遇到空行重置跳過狀態
    ...
    return result if result else text  # 避免過濾後變空白
