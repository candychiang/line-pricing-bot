...
LOG_SHEET_ID = os.environ.get("GOOGLE_LOG_SHEET_ID")
...
def get_log_sheet():
    ...
    try:
        ...
        # 當月工作表名稱
        month_name = datetime.now(TW_TZ).strftime("%Y-%m")

        # 嘗試取得當月工作表，不存在則建立
        try:
            ws = spreadsheet.worksheet(month_name)
        except:
            ws = spreadsheet.add_worksheet(title=month_name, rows=1000, cols=5)
            # 加入欄位標題
            ws.append_row(["時間", "業務名稱", "LINE ID", "原始詢價內容", "Bot回覆內容"])
        return ws
    except Exception as e:
        print("取得記錄工作表失敗: " + str(e))
        return None
...
def log_to_sheet(user_id, display_name, user_msg, ai_reply):
    try:
        ...
        ws.append_row([now, display_name, user_id, user_msg, short_reply])
    except Exception as e:
        print("記錄失敗: " + str(e))
...
def check_missing_info(text):
    ...
    city_level = ["台北市","新北市","基隆市","桃園市","新竹市","新竹縣",
                  ...
                  "宜蘭","花蓮","台東"]
    # 地名後接這些字代表是地址的一部分，不算獨立地點
    addr_suffix = ["里","路","街","巷","弄","號","樓","段","鄰","工業區","工業","科技"]
    ...
    for kw in LOCATION_KEYWORDS:
        if kw in city_level:
            continue
        idx = text_for_detection.find(kw)
        if idx == -1:
            continue
        # 關鍵字後面接地址組成字詞 → 不算獨立地點
        after = text_for_detection[idx+len(kw):idx+len(kw)+3]
        if any(after.startswith(s) for s in addr_suffix):
            continue
...
• 🚨 木箱（WDC）請確認是否有堆高機或天車（有WDC且未觸發重量強制時才列）
